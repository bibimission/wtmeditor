<template>
  ...
  <q-btn @click="generateThumbnails(0)" color="orange" v-if="currentPhotoshoot != ''">Generate Thumbnails</q-btn>
  <div v-if="currentPhotoshoot != ''">
    <div class="col imgContainer" :class="{ 'reduced': photoInEdit != '' }">
      <div class="imgGrid">
        <video v-for="img, index in computeCurrentPhotos" :key="index" :src="img" @click="imageClick($event, img)" :class="{ 'wrong': !isGoodFormat(img), 'selected': photoInEdit == img, 'cover': img.split('cover').length > 1 }" @dragstart="dragStart" @drop="dropped" @dragenter="cancelDefault" @dragover="cancelDefault" :name="img"></video>
      </div>
    </div>
  ...
  <div v-if="currentPhotoshoot != ''">
    ...
    <div class="hidden" id="thumbnail_generator">
      <canvas ref="canvas"></canvas>
      <video ref="thumb_video" @loadeddata="onVideoThumbnailLoad" muted></video>
    </div>
  </div>
</template>
<script>
...
export default defineComponent({
  ...
  methods: {
    selectPhotoshoot(e, pts) {
      ...
      window.ipcRenderer.invoke('file:read', { path: pts + "/videoshoot_config.json" }).then((content) => {
    ...
    onChange() {
      ...
      if (this.photoInEdit != '') {
        ...
        newName += this.id + "_" + this.photoType?.value + (tagList.length > 0 ? ('-' + tagList.join(',')) : '') + '.webm'
    ...
    onInfoChange() {
      window.ipcRenderer.send('file:write', { path: this.currentPhotoshoot + "/videoshoot_config.json", text: JSON.stringify(this.photoshootInfos) })
    },
    imageClick(e, fp) {
      if (!this.isGoodFormat(fp)) {
        if (!this.isConverting) {
          ...
          window.ipcRenderer.invoke('img:convert:webm', { img: fp }).then((newPath) => {
            if (newPath) {
    ...
    resetIds() {
      this.computeCurrentPhotos.forEach((p, i) => {
        ...
        window.ipcRenderer.send('img:rename', { oldPath: p, newPath: newName + '.' + p.split('.').slice(0, -1)[0] });
      });
    },
    generateThumbnails(nextIndex = 0) {
      this.$refs.thumb_video.src = ""
      this.currentThumb = nextIndex
      if (nextIndex <= this.computeCurrentPhotos.length) {
        this.$refs.thumb_video.src = this.computeCurrentPhotos[nextIndex]
      }
    },
    onVideoThumbnailLoad() {
      ...
      var video = this.$refs.thumb_video
      video.play()
      ...
      setTimeout(() => {
        ...
        var thumb = canvas.toDataURL("image/webp").split(';base64,')[1]
        window.ipcRenderer.send('file:write', { path: this.currentPhotoshoot + "/" + (this.currentThumb) + "_thumbnail.webp", text: thumb, base64: true })
        video.pause()
        this.generateThumbnails(this.currentThumb + 1)
      }, 1000)
    }
  },
  computed: {
    coputePhotoShootsNames() {
      return this.files.filter(function (p) {
        if (p.includes("/videoshoots/")) {
          return p.split("/videoshoots/")[1].split("/").length == 1;
        }
        return false;
      });
    },
    computeCurrentPhotos() {
      if (this.currentPhotoshoot != '') {
        var photos = this.files.filter(p => p.split(this.currentPhotoshoot).length > 1 && p != this.currentPhotoshoot && p.split(".")[1] != "ini" && p.split(".")[1] != "json" && p.split(".")[1] != "webp")
          .sort((a, b) => {
