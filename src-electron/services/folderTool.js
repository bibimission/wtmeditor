...
class FolderTool {
  ...
  writeGirlInfoFile(girlName, girlObject) {
    if (girlName != "") {
      ...
      this.writeFile(GIRLS_BASE_PATH + "/" + girlName + "/_BK.ini", iniText);
    }
  }
  async readGirlInfoFile(girlName) {
    var file = await this.readFile(
      GIRLS_BASE_PATH + "/" + girlName + "/_BK.ini"
    );
    var lines = file.split("\n");
    var girlObject = {};
    lines.forEach((l) => {
      if (l.split("first_name").length > 1) {
        girlObject.first_name = l
          .split("first_name")[1]
          .trim()
          .substring(1)
          .trim();
